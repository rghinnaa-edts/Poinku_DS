...
class Coachmark: UIView {
    ...
    private func updateContentViewPosition() {
        ...
        let spaceBelow = self.bounds.height - targetFrame.maxY - padding - contentHeight
        ...
        if spaceBelow < 0 {
            ...
            arrowPosition = .top
            contentView.frame.origin.y = targetFrame.maxY + padding
        }
        
        contentView.frame.origin.x = max(16, min(self.bounds.width - contentView.frame.width - 16, targetFrame.midX - contentView.frame.width / 2))
    ...
    private func updateTriangleForArrowPosition() {
        ...
        if arrowPosition == .top {
            ...
            contentView.frame.origin.y += triangleHeight
        } else {
            triangleView.transform = CGAffineTransform.identity
            triangleView.frame.origin.y = contentView.frame.maxY
        }
    ...
    private func updateTrianglePosition() {
        ...
        triangleX = max(contentView.frame.minX + triangleWidth, min(triangleX, contentView.frame.maxX - triangleWidth * 2))
        
        let triangleY = (arrowPosition == .top)
                        ? contentView.frame.minY - triangleHeight
